...
//根据传入的数据维度，获得表中的数据的id
//先查看缓存中是否有数据
//缓存中有数据的话直接拿到缓存数据
//缓存中没有的话查看表中有没有数据
//表中有数据的话，从表中拿到数据，同时放到缓存中
//表中没有的话就插入，再次进行查询缓存到内存中
...
public class DimensionConverterImpl implements DimensionConverter {
    ...
    public DimensionConverterImpl() {
        //jvm关闭时，释放资源
        Runtime.getRuntime().addShutdownHook(new Thread(() -> JDBCutils.close(
                threadLocalConnection.get(), null, null)));
    }
    ...
    public int getDimensionID(BaseDimension dimension) {
        ...
        //时间维度：date_dimension_year_month_day, 2

        //联系人维度：contact_dimension_telephone, 1
        String cacheKey = genCacheKey(dimension);

        //尝试获取缓存的id，如果有对应的key,调用get方法获取对应的id
